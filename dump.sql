...
INSERT INTO `motorcycles_types` (`name`) VALUES
	('Классика'),
	('Спортивные'),
    ('Супербайк'),
    ('Дрэгстер'),
    ...
    ('Минибайк'),
    ('Тяжёлые мотоциклы');
INSERT INTO `motorcycles` (`name`, `discontinued`, `type_id`) VALUES
	('Yamaha V-Max', NULL, 4),
...
SELECT
	...
	, COUNT(`a`.`id`) AS 'count'
FROM
	`motorcycles_types` AS b
LEFT JOIN
	`motorcycles` AS a ON `a`.`type_id` = `b`.`id` AND `a`.`discontinued` IS NULL
GROUP BY `b`.`id`;
